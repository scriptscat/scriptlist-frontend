...
const actions: ActionTree<ScriptsStateInterface, StateInterface> = {
  ...
  async fetchScriptInfo({ commit }, param: { id: number; cookies: Cookies }) {
    try {
      const response = await http.get<API.ScriptInfoResponse>(
        '/scripts/' + param.id.toString(),
        {
          headers: {
            cookie:
              'token=' + (param.cookies ? param.cookies.get('token') : ''),
          },
        }
      );
      if (response.data.code === 0) {
        ...
        commit('updateErrMsg', response.data.msg);
      }
    } catch (error) {
      commit('updateErrMsg', '系统错误');
    }
  },
  fetchVersionList(
    { commit },
    param: { id: number; page: number; count: number; cookies: Cookies }
  ) {
    return fetchVersionList(param)
      .then((resp) => {
